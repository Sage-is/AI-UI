...
- Templating is now decidable. Three fragment surfaces exist, which was the stated trigger. Roughly half of each panel's Python is hand-built HTML strings, and that is what a template engine deletes and nothing else will. Make the decision against these three call sites rather than waiting for a fourth.

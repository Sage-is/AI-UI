...
>
	<Tooltip content={$i18n.t('Add Content')}>
		<button
			style="--p:0.4rem; --radius:0.6rem; --hvr-bgc:var(--color-gray-100); --dark-bgc:var(--color-gray-850); --hvr-dark-bgc:var(--color-gray-800); --tn:color, background-color, border-color, text-decoration-color, fill, stroke, opacity, box-shadow, transform, filter, backdrop-filter 150ms cubic-bezier(0.4, 0, 0.2, 1); --weight:500; --size:0.8rem; --d:flex; --ai:center; --g:0.2rem"
			on:click={(e) => {
				e.stopPropagation();
				show = true;
			}}
		>
			<Icon name="plus-fill-16" className="size-4" />
		</button>
	</Tooltip>

	<div slot="content">
		<DropdownMenu.Content
			style="--w:100%; --maxw:11rem; --radius:0.6rem; --p:0.2rem; --z:50; --bgc:#fff; --dark-bgc:var(--color-gray-850); --dark-c:#fff; --shadow:1"
			sideOffset={4}
			side="bottom"
			align="end"

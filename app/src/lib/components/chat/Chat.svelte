	...
	const loadChat = async () => {
		...
		if (chat) {
			...
			if (chatContent) {
				...
				const taskRes = await getTaskIdsByChatId($chatId).catch((error) => {
					return null;
				});
	...
	const stopResponse = async () => {
		if (taskIds) {
			for (const taskId of taskIds) {
				const res = await stopTask(taskId).catch((error) => {
					toast.error(`${error}`);
					return null;
				});
			}
...
<div
	style="--h:100vh; --maxh:100dvh; --tdn:200ms; --ttf:cubic-bezier(0.4, 0, 0.2, 1); --w:100%; --maxw:100%; --d:flex; --fd:column; --transition:width var(--tdn) var(--ttf); {$showSidebar ? '--maxw:calc(100% - 260px)' : ''}"
	id="chat-container"
>
	{#if !loading}

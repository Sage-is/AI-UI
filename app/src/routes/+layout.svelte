<script lang="ts">
				...
				if (done) {
					if ($settings?.notificationSoundAlways ?? false) {
						...
						audio
							.play()
							.catch((error) => {
								console.log('Notification audio not available:', error.message);
							})
							.finally(() => {
	...
	const checkTokenExpiry = async () => {
		...
		if (now >= exp - TOKEN_EXPIRY_BUFFER) {
			...
			user.set(null);
			// Token is cookie-based, so just redirect to auth
			location.href = res?.redirect_url ?? '/auth';
		}
	};

	// Reactive statement to sync SSR data to stores
	$: if (data) {
		if (data.user) {
			user.set(data.user);
		}
		if (data.config) {
			...
			WEBUI_NAME.set(data.config.name);
		}
	}

	$: if ($config) {
		const lang = $config.default_locale ? $config.default_locale : 'en-US';
		// Initialize i18n

<script>
	...
	import ToolkitEditor from '$lib/components/workshop/Tools/ToolkitEditor.svelte';
	...
	onMount(() => {
		window.addEventListener('message', async (event) => {
			if (
				!['https://sage.is', 'https://www.sage.is', 'http://localhost:9999'].includes(
					event.origin
				)
			)
				return;

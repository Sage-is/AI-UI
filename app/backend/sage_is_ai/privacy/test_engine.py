...
from sage_is_ai.privacy.engine import (
    MemoryMapper,
    StreamReverser,
    pseudonymize,
    reverse,
...
class EngineTest(unittest.TestCase):
    ...
    def test_detectors_pseudonymize_with_shape_and_the_card_is_redacted(self):
        ...
        self.assertEqual(
            {h.rule for h in hits}, {"phone", "email", "postal_ca", "card"}
        )
    ...
    def test_literal_rules_and_hints_catch_names_and_streets(self):
        ...
        out, _ = pseudonymize(
            "Faye Silva lives at 12 Alder Ave",
            rules,
            self.mapper,
    ...
    def test_redact_is_one_way_and_literal_reverses_when_unique(self):
        ...
        self.assertEqual(
            reverse(out, self.mapper.reverse_table()), "Acme Realty called"
        )
    ...
    def test_stream_reversal_survives_a_fake_split_across_chunks(self):
        out, _ = pseudonymize(
            "Reply to faye.silva@example.com about (613) 555-0187 today",
            self.rules,
            self.mapper,
        )
        table = self.mapper.reverse_table()
        pieces, emitted, reverser = (
            [out[i : i + 5] for i in range(0, len(out), 5)],
            "",
            StreamReverser(table),
        )
        for piece in pieces:
            emitted += reverser.feed(piece)
        emitted += reverser.flush()
        self.assertEqual(
            emitted, "Reply to faye.silva@example.com about (613) 555-0187 today"
        )
    ...
    def test_pseudonymizing_twice_is_stable(self):
        ...
        self.assertEqual(
            len(self.mapper.by_fake), 3
        )  # phone, email, postal; the card is redacted
    ...
    def test_a_fake_never_repeats_the_real_name_or_street(self):
        for key in (
            ...
            for surname in ("Silva", "Moreau", "Okafor", "Andersen"):
                self.assertNotEqual(
                    mapper.fake("surname", surname, "pseudonym").lower(),
                    surname.lower(),
                    key,
                )
    ...
    def test_collisions_get_a_new_fake(self):
        ...
        self.mapper.by_fake[a] = (
            "email",
            "someone-else",
        )  # force a collision on the next call
...
class BadDetectorTest(unittest.TestCase):
    ...
    def test_an_unknown_detector_matches_nothing_rather_than_raising(self):
        self.assertEqual(
            Rule("x", "detector", "not-a-detector").compiled().findall("anything"), []
        )

...
class TestAuths(AbstractPostgresTest):
    ...
    def test_update_password(self):
        ...
        old_auth = self.auths.authenticate_user(
            "john.doe@sage.is", "old_password"
        )
        assert old_auth is None
        new_auth = self.auths.authenticate_user(
            "john.doe@sage.is", "new_password"
        )
    ...
    def test_signin(self):
        ...
        response = self.fast_api_client.post(
            ...
            json={"email": "john.doe@sage.is", "password": "password"},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["id"] == user.id
        assert data["name"] == "John Doe"
        assert data["email"] == "john.doe@sage.is"
        assert data["role"] == "user"
        assert data["profile_image_url"] == "/static/user.png"
    ...
    def test_add_user(self):
        ...
        assert data["email"] == "john.doe2@sage.is"

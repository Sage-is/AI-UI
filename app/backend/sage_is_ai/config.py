...
class AppConfig:
    ...
    def __init__(
        ...
        redis_key_prefix: str = "sage-is-ai",
    ):
...
# Passwordless login for existing users via emailed link.
# Admin enables it and configures SMTP.
####################################

ENABLE_MAGIC_LINK_LOGIN = PersistentConfig(
    "ENABLE_MAGIC_LINK_LOGIN",
    "magic_link_login.enable",
    os.environ.get("ENABLE_MAGIC_LINK_LOGIN", "False").lower() == "true",
)

MAGIC_LINK_SMTP_HOST = PersistentConfig(
    "MAGIC_LINK_SMTP_HOST",
    "magic_link_login.smtp_host",
    os.environ.get("MAGIC_LINK_SMTP_HOST", ""),
)

MAGIC_LINK_SMTP_PORT = PersistentConfig(
    "MAGIC_LINK_SMTP_PORT",
    "magic_link_login.smtp_port",
    int(os.environ.get("MAGIC_LINK_SMTP_PORT", "587")),
)

MAGIC_LINK_SMTP_USER = PersistentConfig(
    "MAGIC_LINK_SMTP_USER",
    "magic_link_login.smtp_user",
    os.environ.get("MAGIC_LINK_SMTP_USER", ""),
)

MAGIC_LINK_SMTP_PASSWORD = PersistentConfig(
    "MAGIC_LINK_SMTP_PASSWORD",
    "magic_link_login.smtp_password",
    os.environ.get("MAGIC_LINK_SMTP_PASSWORD", ""),
)

MAGIC_LINK_SMTP_FROM = PersistentConfig(
    "MAGIC_LINK_SMTP_FROM",
    "magic_link_login.smtp_from",
    os.environ.get("MAGIC_LINK_SMTP_FROM", ""),
)
...
STATIC_DIR = Path(os.getenv("STATIC_DIR", SAGE_IS_AI_DIR / "static")).resolve()
...
if CUSTOM_NAME:
    try:
        r = requests.get(f"https://api.sage.is/api/v1/custom/{CUSTOM_NAME}", timeout=10)
        data = r.json()
        if r.ok:
            if "logo" in data:
                WEBUI_FAVICON_URL = url = (
                    f"https://api.sage.is{data['logo']}"
                    if data["logo"][0] == "/"
                    else data["logo"]
                )
            ...
            if "splash" in data:
                url = (
                    f"https://api.sage.is{data['splash']}"
                    if data["splash"][0] == "/"
                    else data["splash"]
                )
                ...
                r = requests.get(url, stream=True, timeout=30)
                if r.status_code == 200:
                    with open(f"{STATIC_DIR}/icons/splash.png", "wb") as f:
                        r.raw.decode_content = True
                        shutil.copyfileobj(r.raw, f)
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        ...
        OLLAMA_BASE_URL = "http://ollama-service.sage-is-ai.svc.cluster.local:11434"
...
DEFAULT_SIMPLE_REDIRECTS = [
    {
        "source": "/favicon.png",
        "destination": "/static/icons/favicon.png",
        "status_code": 302,
        "preserve_query": False,
    }
]

simple_redirects_env = os.environ.get("SIMPLE_REDIRECTS", "").strip()

if simple_redirects_env:
    try:
        simple_redirects_config = json.loads(simple_redirects_env)
    except Exception as e:
        log.warning(f"Invalid SIMPLE_REDIRECTS value, falling back to defaults: {e}")
        simple_redirects_config = DEFAULT_SIMPLE_REDIRECTS
else:
    simple_redirects_config = DEFAULT_SIMPLE_REDIRECTS

SIMPLE_REDIRECTS = PersistentConfig(
    "SIMPLE_REDIRECTS",
    "ui.simple_redirects",
    simple_redirects_config,
)
...
USER_PERMISSIONS_WORKSHOP_KNOWLEDGE_ACCESS = (
    os.environ.get("USER_PERMISSIONS_WORKSHOP_KNOWLEDGE_ACCESS", "False").lower()
    == "true"
)

USER_PERMISSIONS_WORKSHOP_PROMPTS_ACCESS = (
    os.environ.get("USER_PERMISSIONS_WORKSHOP_PROMPTS_ACCESS", "False").lower()
    == "true"
)

USER_PERMISSIONS_WORKSHOP_TOOLS_ACCESS = (
    os.environ.get("USER_PERMISSIONS_WORKSHOP_TOOLS_ACCESS", "False").lower() == "true"
)

USER_PERMISSIONS_WORKSHOP_MODELS_ALLOW_PUBLIC_SHARING = (
    os.environ.get(
        "USER_PERMISSIONS_WORKSHOP_MODELS_ALLOW_PUBLIC_SHARING", "False"
    ).lower()
    == "true"
)

USER_PERMISSIONS_WORKSHOP_KNOWLEDGE_ALLOW_PUBLIC_SHARING = (
    os.environ.get(
        "USER_PERMISSIONS_WORKSHOP_KNOWLEDGE_ALLOW_PUBLIC_SHARING", "False"
    ).lower()
    == "true"
)

USER_PERMISSIONS_WORKSHOP_PROMPTS_ALLOW_PUBLIC_SHARING = (
    os.environ.get(
        "USER_PERMISSIONS_WORKSHOP_PROMPTS_ALLOW_PUBLIC_SHARING", "False"
    ).lower()
    == "true"
)

USER_PERMISSIONS_WORKSHOP_TOOLS_ALLOW_PUBLIC_SHARING = (
    os.environ.get(
        "USER_PERMISSIONS_WORKSHOP_TOOLS_ALLOW_PUBLIC_SHARING", "False"
    ).lower()
    == "true"
)
...
DEFAULT_USER_PERMISSIONS = {
    "workshop": {
        "models": USER_PERMISSIONS_WORKSHOP_MODELS_ACCESS,
        "knowledge": USER_PERMISSIONS_WORKSHOP_KNOWLEDGE_ACCESS,
        "prompts": USER_PERMISSIONS_WORKSHOP_PROMPTS_ACCESS,
        "tools": USER_PERMISSIONS_WORKSHOP_TOOLS_ACCESS,
    },
    "sharing": {
        "public_models": USER_PERMISSIONS_WORKSHOP_MODELS_ALLOW_PUBLIC_SHARING,
        "public_knowledge": USER_PERMISSIONS_WORKSHOP_KNOWLEDGE_ALLOW_PUBLIC_SHARING,
        "public_prompts": USER_PERMISSIONS_WORKSHOP_PROMPTS_ALLOW_PUBLIC_SHARING,
        "public_tools": USER_PERMISSIONS_WORKSHOP_TOOLS_ALLOW_PUBLIC_SHARING,
    },
    "chat": {
...
QDRANT_COLLECTION_PREFIX = os.environ.get("QDRANT_COLLECTION_PREFIX", "sage-is-ai")
...
PINECONE_INDEX_NAME = os.getenv("PINECONE_INDEX_NAME", "sage-is-ai-index")

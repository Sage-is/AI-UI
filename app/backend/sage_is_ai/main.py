...
from sage_is_ai.utils import logger
from sage_is_ai.utils.audit import AuditLevel, AuditLoggingMiddleware
from sage_is_ai.utils.logger import start_logger
...
from sage_is_ai.routers import (
    audio,
    bridges,
    images,
    ollama,
...
from sage_is_ai.internal.db import Session, engine
...
from sage_is_ai.utils.models import (
    get_all_models,
    get_all_base_models,
    check_model_access,
)
from sage_is_ai.utils.chat import (
    generate_chat_completion as chat_completion_handler,
    chat_completed as chat_completed_handler,
    chat_action as chat_action_handler,
)
from sage_is_ai.utils.embeddings import generate_embeddings
from sage_is_ai.utils.middleware import process_chat_payload, process_chat_response
...
from sage_is_ai.utils.plugin import install_tool_and_function_dependencies
from sage_is_ai.utils.oauth import OAuthManager
from sage_is_ai.utils.security_headers import SecurityHeadersMiddleware
from sage_is_ai.utils.redis import get_redis_connection

from sage_is_ai.tasks import (
...
from sage_is_ai.utils.redis import get_sentinels_from_env
...
if ENABLE_OTEL:
    from sage_is_ai.utils.telemetry.setup import setup as setup_opentelemetry
...
app.state.config.SIMPLE_REDIRECTS = SIMPLE_REDIRECTS
...
app.add_middleware(RedirectMiddleware, config=app.state.config)
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for Sage.is AI.
...
async def get_opensearch_xml():
    ...
    <Image width="16" height="16" type="image/x-icon">{app.state.config.WEBUI_URL}/static/icons/favicon.png</Image>
...
# At the moment the file is in /backend/build/static/robots.txt
# So we need to serve it from there.
@app.get("/robots.txt")
async def robots_txt():
    search_paths = [
        STATIC_DIR / "robots.txt",
        FRONTEND_BUILD_DIR / "static" / "robots.txt",
    ]
    ...
    # Fallback: make it obvious in logs while still returning a valid robots payload
    log.warning(
        "robots.txt not found in any search paths",

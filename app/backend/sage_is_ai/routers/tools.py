...
from sage_is_ai.utils.plugin import load_tool_module_by_id, replace_imports
from sage_is_ai.config import CACHE_DIR
from sage_is_ai.constants import ERROR_MESSAGES
from fastapi import APIRouter, Depends, HTTPException, Request, status
from sage_is_ai.utils.tools import get_tool_specs
...
async def create_new_tools(
    ...
    if user.role != "admin" and not has_permission(
        user.id, "workshop.tools", request.app.state.config.USER_PERMISSIONS
    ):
        raise HTTPException(

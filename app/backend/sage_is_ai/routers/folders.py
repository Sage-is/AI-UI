...
def safe_execute(operation, error_msg: str, folder_id: str = ""):
    ...
    try:
        ...
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, ERROR_MESSAGES.DEFAULT(error_msg)
        )
...
def ensure_unique_name(
    parent_id: Optional[str], user_id: str, name: str, exclude_id: str = None
):
    """Ensure folder name is unique in parent."""
    existing = Folders.get_folder_by_parent_id_and_user_id_and_name(
        parent_id, user_id, name
    )
    if existing and existing.id != exclude_id:
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, ERROR_MESSAGES.DEFAULT("Folder already exists")
        )
    ...
    return [
        {
            **folder.model_dump(),
            "items": {
                "chats": [
                    {"title": c.title, "id": c.id, "updated_at": c.updated_at}
                    for c in Chats.get_chats_by_folder_id_and_user_id(
                        folder.id, user.id
                    )
                ]
            },
...
def create_folder(form_data: FolderForm, user=Depends(get_verified_user)):
    ...
    return safe_execute(
        lambda: Folders.insert_new_folder(user.id, form_data), "Error creating folder"
    )
...
async def update_folder_name(
    ...
    return safe_execute(
        lambda: Folders.update_folder_by_id_and_user_id(id, user.id, form_data),
...
async def update_folder_parent(
    ...
    return safe_execute(
        lambda: Folders.update_folder_parent_id_by_id_and_user_id(
...
async def delete_folder(request: Request, id: str, user=Depends(get_verified_user)):
    ...
    return safe_execute(
        lambda: Folders.delete_folder_by_id_and_user_id(id, user.id),

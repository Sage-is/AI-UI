...
A gate that cannot fail is worse than no gate, and this chart has now met the shape twice. The social-tag spec passed for a day while never emitting a tag, because the og assertions only run when `WEBUI_URL` is set and the harness did not pass it; the marquee spec passed while the heading sat still, because a headless run starts with an empty cache and could not reproduce a staleness bug a person hit. Both are fixed. Before trusting any check on this chart, ask what makes it go red.
...
- [ ] **Social preview**: **the machinery is proven; only the taste call is left.** Verified end to end 2026-08-03 against a container with `WEBUI_URL` set — 11 og tags emitted, `og:image` absolute, and fetching that URL returns **200 `image/jpeg`**. The image is exactly 1200×630, matching the declared dimensions, so a crawler crops nothing, and it survives into the runtime image (`static-runtime` strips only pyodide and the CJK fonts). With `WEBUI_URL` unset the whole block is omitted rather than half-emitted, which is the invariant the guard-rail was written around. The block lands on the server-rendered page only, so a self-hosted instance does not advertise itself as Sage.is.
  Two things to judge, and neither is a code question. **The image**: 1200×630, 66 kB, rebuilt from the landing page's own look — library backdrop, the same two-layer dim, circular mark, serif heading — via `tools/og-card/card.html`. Rough, not final art: the mark is `favicon.png` scaled up and the type is Georgia. **Mark and type enlarged 2026-08-03** (Alexander: _"the social share logo and text should be a little larger"_) — mark 132 → 180px, heading 3.5 → 4.5rem, body 1.6 → 2.1rem; judged at 500px, which is the width a feed renders it at. **The copy**: the card reads _"A hosted trial of Sage.is AI, the open-source AI platform for workshops and teams. By invitation only."_ — deliberately not the on-page tagline, because a share card is read by someone who does not have the link yet. Say whether both hold. #prototype
...
- [ ] **Ship and prove**: the tag is decided; what remains is who cuts it, when, and what evidence closes it. The release runbook and the three falsifiable checks are written into the version-jump record. **A 200 proves nothing** — the SPA catch-all answers 200 with the 6,532-byte shell for any unregistered path. Name the cut-off time for landing scope on the release branch, given Friday. #interview
  One check to add to that list, now that the tags are proven locally: the card only exists when `WEBUI_URL` is set, and the omission is invisible — the page looks identical either way and only a crawler notices. **Read live 2026-08-03: it IS set, to `https://try.sage.is`** (the public persona endpoint builds its login URLs from it). So the tags should appear on their own the moment 3.1.0 boots. Confirm by reading the deployed `/` for `property="og:` rather than assuming it.
  Blocked by [Social preview], [Four broken agents], [School B's residue]. The first is now a glance at an image and a sentence rather than a build.
